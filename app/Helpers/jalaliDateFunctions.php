...
function jchecktime($hour, $minute, $second){
...
{
    return jdate('Y/n/j H:i:s',$timestamp,'','','en');
}
function GregorianToJalali($gregorian_date,$format = 'Y/m/d H:i:s'){
    return jdate($format,strtotime($gregorian_date),'','','en');
}
function JalaliToGregorian($jalali_date, $separate = '/')
{
    if (is_null($jalali_date))
        return null;
    $timestamp = JalaliToTimestamp($jalali_date, $separate);
    return \Carbon\Carbon::createFromTimestamp($timestamp);
}

function JalaliToTimestamp($jalali_date, $separate = '/')
{
    $date = explode($separate, substr($jalali_date,0,10));
    $time = substr(tr_num($jalali_date), 11);
    $gre_date = jalali_to_gregorian($date[0], $date[1], $date[2]);
    return strtotime(implode($separate, $gre_date) . ' ' . $time);
}
function getDateType($date, $separate = '/')
{
    $sep_date = explode($separate,tr_num($date));
    if ($sep_date[0] > 1940 && $sep_date[0] < 2100)
        return 'gregorian';
    elseif ($sep_date[0] > 1320 && $sep_date[0] < 1450)
        return 'jalali';
    else
        return false;
}
